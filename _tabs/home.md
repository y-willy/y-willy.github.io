---
layout: page
title: Home
permalink: /
---

# 황윤성 (Hwang Youn Sung)

> 취약점 분석 · 보안 · Problem Solving 중심 개발자

---

## 학력
- **경희대학교 / 컴퓨터공학과** (2023.03 ~ 현재)
- **인하대학교 / 통계학과** (2023.03 ~ 2023.12) 

---

## 기술 스택

### Language
- C++, C
- Python

### System
- Linux

### Tools
- Git

### Problem Solving
- Baekjoon: (골4)
- Solved.ac: (https://solved.ac/profile/willy)

---

## 프로젝트

### 프로젝트 이름
- **기간:** 2023.04 ~ 2023.06  
- **설명:**  
  → 대한민국 국민들의 여가활동 데이터 분석  
- **사용 기술:**  
  → Python, Pandas, Matplotlib, Excel 데이터 전처리
- **주요 기능:**  
  → 공공데이터 수집 및 전처리, 성별/연령/소득별 참여율 비교, 가설 설정 및 검정
- **성과 / 배운 점:**  
  → 데이터 전처리 및 통계적 가설 검정 경험

---

### 프로젝트 이름 2
- **기간:**  
- **설명:**  
- **사용 기술:**  
- **주요 기능:**  
- **성과 / 배운 점:**  

---

## 🏆 수상 경력
- **2025년 KHU 가을 프로그래밍 경시대회 / 장려상** (2025) [상장](/assets/2025.pdf)
- **2025 후마니타스백일장 / 장려상** (2025) [상장]()

---

## 대외 활동
- **버그헌팅 실습 훈련 - 초급과정 [12차]** [수료증](/assets/bug.pdf)
  - 기간: 2025.08.04 ~ 2025.08.16 
  - 내용: 웹해킹 종류 및 탐지 기법 학습
- **성균관대 AX 사이버보안 특강** [수료증]()
  - 기간:  
  - 내용: 

---

## 동아리 / 스터디
- **이름**
  - 내용:

---

## 📌 기타
- **GitHub:** https://github.com/y-willy
- **Blog:** https://y-willy.github.io  
- **Email:** willy0723@naver.com